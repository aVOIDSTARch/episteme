# Scraped: kotlin-android
**Source:** https://developer.android.com/kotlin/style-guide
**Fetched:** 2026-03-03T17:39:01.721680+00:00
...
should reflect the case-sensitive name plus the
...
acceptable if the filename is plural), and append the
...
Poor: there’s no reason to use an escape with a printable character.
...
Wildcard imports (of any type) are
...
file can declare one or more types, functions, properties, or type
...
What is important is that each file uses

some

logical order, which its
...
which have no more than one

else

branch and which fit on a single line.
...
Braces are otherwise required for any
...
statements and expressions, even when the body is empty or contains only
...
that brace terminates a

statement or terminates the body of a function, constructor, or
...
conditional that is used as an expression may

omit braces

only

if the entire expression fits on one line.
...
(for example, a long URL in KDoc)
...
When a line is broken at an operator or infix function name, the break comes

after the operator or infix function name.
...
The primary goal for line wrapping is to have clear code, not
...
consecutive members of a class: properties, constructors,
...
A blank line between two
      ...
      with their backing property, if present.

Exception:

Blank lines between enum constants are covered
...
) only if used in a class declaration for specifying

a base class or interfaces, or when used in a
...
, are not used except in the case of
...
also be nouns or noun phrases (for example,
...
Function names are written in camelCase and are typically verbs or verb phrases. For example,
...
with words separated by underscores. But what
...
properties with no custom

get

function, whose contents are
...
as well as scalars and string if marked as

const

. If any of an instance’s
...
Constant values can only be defined inside of an

object

or as a top-level declaration. Values otherwise meeting the requirement of a

constant but defined inside of a

class

must use a non-constant name.

Constants which are scalar values must use the
...
if any word already has a conventional camel-case appearance in common usage, split this into its constituent parts (e.g., “AdWords” becomes “ad words”). Note that a word such as “iOS” is not really in camel case per se; it defies any convention, so this recommendation does not apply.
...
Uppercase the first character of each word except the first to yield
...
, and these never appear with an empty description.
...
At the minimum, KDoc is present for every
...
KDoc is optional for “simple, obvious” functions like
...
) if a typical reader may have no
