...
**Fetched:** 2026-03-03T17:38:54.516782+00:00
...
, also known as readability, is what we call
...
serve. These are the fundamental
...
exceptions, such as unit tests and small
...
these definitions to separately included header (
...
instantiations of a template occur in one
...
or because the definition is accessible to only
...
, and might not include

their prerequisites.  Name such files with the
...
symbol's definition is exposed to the reader at

  the point of declaration.

A function or variable defined in a header file is
...
section of a class, within a namespace that includes the word
...
Once a definition is in a header file, it must be ODR-safe by having the
...
Note that the C headers such as
...
namespace, so give each library within a
...
them in an unnamed namespace or declaring them

static

.

Do not use either of these constructs in
...
Do not use internal linkage in
...
declaration, considered in isolation, could be
...
function-local variables that are declared with the
...
process ID.) The other kind of initialization is
...
happens to objects with static storage duration
...
Fundamental types (like pointers and
...
If code triggered by the destructor of any

thread_local

variable

refers to any already-destroyed
...
s. Specifically, skipping destructors for globals and static
    ...
    contrast, skipping destructors for
...
we will suffer from hard to diagnose use-after-free bugs.
    ...
    minimize the potential of accessing any other
...
object of one type (called the

source type

) to

be used where a different type (called the

destination

type

) is expected, such as when passing an
...
language treats it as one as far as
...
parameter takes the
    ...
    place of separate overloads for
...
incorrect. Types representing singleton objects (

Registerer

),

objects tied to a specific scope (
...
declaring and/or deleting the appropriate operations in the
...
or an interface-only base class,
...
only for passive objects that
      ...
      carry data; everything else is a
...
While using pairs and tuples can avoid the need to define a custom type,
  ...
  potentially saving work when

writing

code, a meaningful field

  name will almost always be much clearer when
...
When using inheritance, make it
...
destructors with exactly one of an
...
Multiple inheritance is permitted, but multiple
...
we ban
  ...
  using-declarations (which

we ban in header files
...
If there is a single obvious notion of when a value
...
is less than another such value
...
ordering, but you want to store it in a
...
of some easy boilerplate in the form of accessors (usually
...
reasons, we allow data members of a test fixture class defined in a
...
If a test fixture class is defined outside of the
...
Group similar declarations together, placing
...
which member of the overload set is chosen, only that
...
value. Follow the same restrictions as for
...
semantics of overloaded functions, so all the
...
The trailing return type is in the function's scope. This doesn't
  ...
  make a difference for a simple case like
...
It cannot be copied, but can be
...
escapes from a function marked
...
instead of writing a complicated
...
run-time. This is done by use of
...
exemplified by the standard header
...
streams API (other than error state), such as
...
unless they
  ...
  that state, e.g., by returning a non-
...
to be used in defining a

constexpr

variable. Functions can be declared
...
Prematurely marking something as
...
Current restrictions on what is allowed in
...
on the size of an integer. Prefer to omit the
...
rather than copying the in-memory representation around.
...
For pointers (address values), use
...
Use return type deduction (for both functions and lambdas) only if the
  ...
  function body has a very small number of
...
the interface. In particular, public functions in header files
...
, which largely supersedes the general rules for
...
would not. Structured bindings are especially
  ...
  beneficial when the object is a pair or tuple (as in the

insert

example above), because they don't have meaningful field names to begin with,

  but note that you generally

shouldn't use

    pairs or tuples

unless a pre-existing API like
...
(often abbreviated "CTAD") occurs when
...
Explicit deduction guides look like function declarations with trailing
  ...
  return types, except that there's no leading
...
, and the
  ...
  relies on this deduction guide for
...
CTAD also suffers from many of the same drawbacks as
...
Do not use CTAD with a given template unless the template's maintainers
  ...
  deduction guide (all templates in the
...
are a syntax that allows for initializing an
...
The explicitly listed fields will be initialized as specified, and others
  ...
  initialization expression like
...
according to C++20 to initialize
...
, and suffer from many of the same drawbacks as
...
class members in its body must capture
...
) which determines coroutine parameter types, how coroutines
...
Like other declarations, aliases declared in a header file are part of that
  ...
  or in an explicitly-marked internal namespace. Aliases in such areas or in
...
make it easier to change the type later.
...
sections of classes, explicitly-marked internal namespaces, and in
...
), and "it"/"its" for software, computers, and other
...
capitalize it as a single "word", e.g.,
...
definition and the words within the names. For example,
...
local pattern to follow, prefer "
...
Concept names follow the same rules as
...
for a discussion of when to use a struct
...
for details) should be named this way, including those that are static constant
...
unqualified Aliases are generally banned
...
more common. Be consistent with how you
...
Private methods and functions declared in
...
and should start with the verb phrase; for example,
...
than in C++17, producing arrays of
...
condition or iteration specifier
...
for a discussion of what comments are
...
for rules on ordering declarations within
...
, or to attach a comment to only
...
compiler option to include the
...
and contain only macros, do not
