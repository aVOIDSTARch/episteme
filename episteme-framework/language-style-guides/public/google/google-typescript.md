# Scraped: google-typescript
**Source:** https://google.github.io/styleguide/tsguide.html
**Fetched:** 2026-03-03T17:38:56.215260+00:00
...
All examples given are

non-normative

and serve only to illustrate the

normative language of the style guide. That is, while the examples are in Google

Style, they may not illustrate the
...
corresponding numeric escape (e.g
...
separates each section that is present.
...
use paths to import other TypeScript code. Paths
...
have clear names, for example Jasmine's
...
be aliased to clearer names as needed with
...
exported symbols that have common names like
...
fix name collisions by using a namespace import or renaming the

exports themselves. Code
...
be more readable when renamed to
...
something that hasn't been declared. In
...
also have a perhaps needless second scope (the class
...
to paraphrase this style point is that
...
Note: If you need to force a runtime load for side effects, use
...
refer to code in other files using imports and exports of
...
only be used when required to interface with external, third party code.
...
Language features which are not discussed in this style guide
...
a variable needs to be reassigned. Never use
...
When using spread syntax, the value being spread
...
Either filter values explicitly with an
...
In contrast, statements that contain class expressions
...
use a semicolon to separate individual
...
rely on dynamic dispatch of static

methods. Static methods

should

only be called on the base class

itself (which defines it directly). Static methods
...
be called directly on a subclass that doesn’t define the method
...
JavaScript allows accessing static fields through
...
use parentheses, even when no arguments are passed:
...
modifiers or parameter decorators

should not

be omitted even if the body of
...
as an Angular component's properties used from a template,
...
Angular and AngularJS template properties should use
...
Getters and setters, also known as accessors, for class members
...
consistent and has no side effects: getters
...
prefixed or suffixed with any whole word, like
...
possible. At least one accessor for a property

must

be non-trivial: do not
...
Instead, make the property public (or consider making it
...
rationale for not mixing key types
...
: Framework code (such as Polymer, or Angular) may need to use
...
passed as a parameter, using the
...
be used, for example, when an explicit type annotation is
...
Functions nested within other methods or functions
...
arrow functions are preferred because they have access to the outer
...
The block body makes sure the return type is
...
specifically exist to rebind the
...
Prefer arrow functions over other approaches to binding
...
contain properties initialized to arrow functions.
...
is already bound, which increases confusion about what

this

is, and call sites and references using such handlers look broken (i.e. require

non-local knowledge to determine that they are correct). Code
...
obtain or pass references to instance
...
use arrow functions when there is no need to uninstall the
...
approach, because they automatically capture
...
be given a default initializer to use when

the argument is omitted. Initializers

must not

have any observable side
...
parameter instead of accessing
...
Use function spread syntax instead of
...
be used within function bodies sparingly to create
...
function func(this: ThisType, ...)

), or in

arrow functions defined in a scope where
...
to refer to the global object, the context of an

eval

, the

target of an event, or unnecessarily
...
(that is, ending a line inside a string literal
...
and so situations where it

does

matter are undesirable because they will be
...
Code that requires parsing with a radix

must

check that its input contains

only appropriate digits for that radix before calling into
...
boolean coercion. Those are the conditions in an
...
As with explicit conversions

, values of enum types (including

unions of enum types and other types)
...
statements fitting on one line
...
unwanted prototype properties. Prefer
...
from functions. They should be defined and used wherever the native
...
arbitrary values. However if the thrown or rejected value is not an
...
Instead, only throw (subclasses of)
...
When catching errors, code

should

assume that all thrown errors are instances
...
since this will catch the error thrown by
...
fall through (enforced by the compiler):
...
) and non-nullability assertions (
...
use type and non-nullability assertions
...
assertion form is safe. In those situations, you
...
not be necessary. For example, generated proto code is always
...
version of a type. Adding a type assertion (

x as Foo

) which

does not meet this criteria will give the error:

Conversion of type 'X' to type

'Y' may be a mistake because neither type sufficiently overlaps with the other.

If you are sure an assertion is safe, you can perform a

double assertion

. This

involves casting through
...
There may be performance issues if try blocks are inside a loop.
...
When using decorators, the decorator

must

immediately precede the symbol it
...
instantiate the wrapper classes for the primitive
...
TypeScript enums already cannot be mutated;
...
use only ASCII letters, digits, underscores (for constants
...
be descriptive and clear to a new reader. Do not use abbreviations
...
Variables that are in scope for 10 lines or fewer, including

arguments that are

not

part of an exported API,
...
use a single upper case character
...
Test method names inxUnit-style test frameworks
...
be used as an identifier by itself (e.g. to
...
be used for values that can technically be modified (i.e.
...
: Only symbols declared on the module level, static fields of module

level classes, and values of module level enums,
...
existing identifier. The local alias

must

match the existing naming and format

of the source. For variables use
...
rely on type inference as implemented by the TypeScript compiler for
...
from being inferred as

unknown

. For example, initializing generic types with

no values (e.g. empty arrays, objects,
...
the code author. Reviewers

may

ask for annotations to clarify complex return

types that are hard to understand. Projects

may

have a local policy to always
...
is no special syntax for unions of
...
TypeScript code can use either
...
original issue that resulted in
...
deal with null values close to where they arise, using the
...
Optional parameters implicitly include
...
Honestly, my take is that it should
...
These rules apply at each level of nesting, i.e. a simple

T[]

nested in a more

complex type would still be spelled as
...
s can contain—types other than

string

.

TypeScript's builtin
...
includes several type operators based on these (
...
compile or seem to give the right results. This increases
...
type by subsetting another type
...
dereferencing all properties. As such,
...
In circumstances where you want to use
...
just needs to express that a type is unknown. Use the built-in type
...
better type safety by being explicit about the type
...
There are a few types related to JavaScript primitives that
...
have slightly different meaning from the
...
, but is slightly looser.

Use

{}

for a type that include everything except
...
to further exclude the other primitive types (the
...
They superficially seem to be an easy way to
...
in unit tests, though you generally
...
Google TypeScript includes several
...
If a single-line comment overflows into multiple lines, it
...
optimization. As such, these comments
...
JSDoc is written in Markdown, though it

may

include HTML when necessary.
...
Line-wrapped block tags are indented four spaces. Wrapped description text
...
code. Avoid merely restating the property or parameter name. You
...
Symbols that are only exported to be consumed by tooling, such as
...
third person, as if there is an implied
...
, but also declares a private readonly property
...
To document these fields, use JSDoc's
...
parameter is enough. Code will

usually

benefit from more documentation than
...
what the other code in the same file is already doing (
...
use Google Style, regardless of the style choices of
...
. If this reformatting is not

done, then new code

should

be as consistent as possible with existing code in
...
Mark deprecated methods, classes or interfaces with an
...
decisions be made locally. So the default answer should be
