# Scraped: google-csharp
**Source:** https://google.github.io/styleguide/csharp-style.html
**Fetched:** 2026-03-03T17:38:54.828193+00:00
...
declarations go at the top, before any namespaces.

using

import order is alphabetical, apart from
...
// `using` goes at the top, outside the
...
// Object initializers use a 2 space
...
Generator code that is directly turned into a container via
...
here is a property that returns a
...
for returns that are not also inputs.
...
should be used rarely, when mutating an input is necessary.
...
to pass a modifiable container into a method.
...
and return types (keeping in mind the guidance on
...
when the size of the container can change.
...
both represent linear, contiguous containers.

Similar to C++ arrays vs
...
In some cases arrays are more performant, but in general
...
If other work needs to be done in the iteration,
