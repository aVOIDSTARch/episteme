# Scraped: google-javascript
**Source:** https://google.github.io/styleguide/jsguide.html
**Fetched:** 2026-03-03T17:38:55.762580+00:00
...
to TypeScript, and following the
...
definition of Google’s coding standards
...
if and only if it adheres to the rules
...
, respectively. Imperative

and declarative statements are prescriptive and correspond to
...
. That is, while the examples

are in Google Style, they may not illustrate the

only

stylish way to represent
...
uses. Filenames’ extension must be
...
depending only on which makes the code
...
separates each section that is present, except the
...
files must declare exactly one

goog.module

name on a single

line: lines containing a
...
that it defines concatenated to the end in
...
Module namespaces may never be named as a

direct

child of another module's
...
be a child or parent of any other
...
object. Exported symbols may be defined directly on the
...
module-local symbols are not declared
...
ES modules are files that use the
...
the same name. Avoid aliasing imports (
...
Non-exported module-local symbols are not declared
...
exception to the 80-column limit. This applies to both
...
exports only a subset of properties from the global
...
in ES modules works as it does in

goog.module

files. You can

require any Closure namespace symbol (i.e., symbols created by
...
module, it can only be called from
...
type annotations, while those imported by a
...
with no empty lines. This block follows the
...
: Types, variables, and functions declared in externs files have to
...
: In certain cases, additional components of the namespace can be
...
prevent masking native types such as
...
in type annotations, it should be imported by a single
...
be wrapped: require lines are an exception to the 80-column
...
: A simple if statement that can fit entirely on a single line with
...
that brace terminates a statement or
...
An empty block or block-like construct

may

be closed immediately after it is
...
4.2.1 Array literals: optionally
...
4.2.2 Object literals: optionally
...
(statements—such as assignments—that contain class

expressions

are still terminated with a semicolon). For inheritance, the
...
types must explicitly specify the template type in an
...
is breaking a chunk of code into multiple
...
A line break is never added between a return and the return value as this
...
before the first or after the last method in a class or object
...
consecutive blank lines are permitted, but never required (nor
...
. Leading whitespace (i.e., indentation) is addressed
...
It is not even required to

maintain

horizontal alignment in places where it
...
default, unless a variable needs to be reassigned. The
...
habitually declared at the start of their containing
...
This works as expected except for the third case: if
...
Object literals may represent either

structs

(with unquoted keys and/or

symbols) or

dicts

(with quoted and/or computed keys). Do not mix these key
...
) in place of a colon immediately followed by a
...
in a method shorthand or

function

refers to the object

literal itself whereas
...
Enums must be module-local or assigned directly on
...
setting any fields or otherwise accessing
...
the proper visibility annotation (
...
subclass constructor (and must be defined with
...
value before calling the superclass
...
appears! For that reason, it is best to use
...
: there are situations where defining a getter or setter is
...
, provided they are defined with
...
import {Converter as FooConverter} from './foo';
...
function a name, it should be assigned to a local
...
for nested functions. Prefer arrow functions over the
...
precedes a single function or method call (using
...
name. Do not name the rest parameter
...
exclude unwanted prototype properties. Prefer
...
exceptional cases occur. Always throw
...
throw string literals or other objects. Always use
...
on the primitive object wrappers (
...
: The package name may conform to TypeScript's path-based pattern. This is
...
Unexported classes are simply locals: they are not marked
...
they are used they should be named
...
static property or a module-local

const

declaration, but not all

@const

static properties and module-local
...
constant. For example, if any of that instance's
...
Aliases may also be used within functions. Aliases must be
...
: When required by a third-party framework, parameter names may
...
identifiers, and must be all-caps, such as
...
into camel case, such as when acronyms or unusual constructs like
...
… each word except the first, to yield
...
are both correct, so the method names
...
constructor. When defining a class
...
even in the case of same-signature
...
annotation. For overridden methods, all
...
For example, methods which return values need a
...
refers to a type annotation for

function types with the keyword
...
Specify parameter and return types with
...
explicitly. Otherwise the default return type is
...
First, fix it or work around it.

Make a strong attempt to actually

address the warning, or find another way to accomplish the task that avoids

the situation entirely.

Otherwise, determine if it's a false alarm.
...
Otherwise, leave a TODO comment.
...
The warning should be visible until it can be
...
@link tags have also been used to create external links in
...
function closures that reference
...
for readability purposes. However, do not put a
...
dependency management is deprecated.
...
even in projects with existing
...
circular dependencies between files in the same library. Unlike
...
. This pattern however is poorly supported
...
fetch the module's export object by calling

goog.module.get('module.name')
...
dependency of your code on the module. The
...
JavaScript file and its consumers from
...
If you can't update consumers of a legacy namespace from
...
declare the module's namespace as a global name just like a

goog.provide()

call does. This allows a non

goog.module

namespace to access the module's

exports without calling
