...
**Fetched:** 2026-03-03T17:38:55.391012+00:00
...
definition of Google's coding standards for

source code in the Java™ Programming Language. A Java source file is described as being
...
that we follow universally, and
...
is used inclusively to mean a normal class, record class, enum

  class, interface or annotation type (
...
(of a class) is used inclusively to mean a nested class, field,

  method,

or constructor

; that is, all top-level contents of a class except initializers.
...
top-level class (of which there is
...
, although Unicode escapes

outside string literals and comments are strongly discouraged.

Tip:

In the Unicode escape case, and occasionally even when actual
...
Never make your code less readable simply out of fear that
...
An ordinary source file consists of these sections,
...
Within each group the imported names appear in ASCII sort order. (

Note:

this is not the same as the import

lines

being in ASCII sort order, since '.'
...
Braces follow the Kernighan and Ritchie style for

nonempty

blocks and block-like
...
that brace terminates a statement or
  ...
  terminates the body of a method, constructor, or
...
). Alternatively, it may be closed immediately
...
(one that directly contains multiple blocks:
...
While the typical reason for line-wrapping is to avoid

overflowing the column limit, even code that would in fact fit within the column limit
...
Extracting a method or local variable may solve the problem
...
The primary goal for line wrapping is to have clear

code,

not necessarily

code that fits in the smallest number of lines.
...
consecutive members or initializers of a class: fields, constructors,
...
Blank lines between enum constants are
...
consecutive blank lines are permitted, but never required (or encouraged).
...
Between any content and a double slash (
...
end of a line; it addresses only
...
This practice is permitted, but is
...
Alignment can aid readability, but attempting to preserve
...
easier to read. It is

not

reasonable to assume that every reader has the entire Java
...
, all other rules for formatting classes apply.
...
Multiple variable declarations are acceptable in the header of a
...
Any array initializer may

optionally

be formatted as if it were a "block-like
...
The square brackets form a part of the
...
For historical reasons, the Java language has two distinct syntaxes for
...
. New-style switches use an arrow
...
and an expression, block, or

throw

. A statement group consists of one or more switch labels each followed by

a colon, then one or more statements, or, for the
...
apply, including the +4 indent for
...
to indicate that execution will or

might

continue into the next statement group. Any
...
Notice that no comment is needed after
...
4.8.4.3 Exhaustiveness and presence of the
...
. That effectively means that every possible value that could be switched on will

be matched by one of the switch labels. A switch is exhaustive if it has a
...
switch to be exhaustive, even those where the language itself does not require it.

This may require adding a
...
), so the indentation level is not
...
annotations (possibly parameterized) may be listed on the same line;
...
. Javadoc is addressed separately in
...
When writing multi-line comments, use the
...
lowercase (to avoid confusion with the digit
...
The contents of a text block may exceed the
...
used. For example, these

names are not Google Style:
...
words are simply concatenated together. For example,
...
class has a name that ends with
...
letters, with each word separated from the next by a single underscore. But what
...
to never mutate the object is not enough. Examples:
...
if any word already has a conventional camel-case appearance in common
    ...
    that a word such as "iOS" is not really in camel case
...
(including acronyms), then uppercase only the first
...
Some words are ambiguously hyphenated in the English
...
formatting of Javadoc blocks is as seen in this example:
...
applies when there are no block tags such as
...
no space after it. HTML tags for other block-level elements, such as
...
Each Javadoc block begins with a brief
...
A common mistake is to write simple Javadoc in the form
...
class, member, or record

component, with a few exceptions noted below. A top-level class is visible if it is

public

; a member is visible if it is
...
is nothing else worthwhile to say but "the foo".
...
Other classes, members, and record components have Javadoc
