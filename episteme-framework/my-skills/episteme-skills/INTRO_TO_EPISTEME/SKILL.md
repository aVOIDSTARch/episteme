# INTRO_TO_EPISTEME

## Name

intro-to-episteme

## Description

Explain what the episteme framework is in this repo, its major components (tracking system, ideas engine, language-style-guides, skills, meta-agent), and how a naive agent should behave inside it.

## When to use

- A new agent is entering this repository for the first time.
- The user or meta-agent asks "what is episteme" or requests a high-level orientation.

## Instructions

1. Read the documents listed in **Read first** to understand the episteme framework and current context.
2. When the trigger in **When to use** occurs, decide whether this skill applies; ask clarifying questions when necessary to reduce unnecessary work.
3. When appropriate, call the skills listed in **REQUIRED_SKILLS** (if any) to perform sub-steps.
4. Apply the conventions from the referenced _docs or notes when creating or updating files.
4. Summarize what you did and what to do next so the next agent can continue.

## Read first

- agent-docs/README.md
- my-skills/episteme-skills/SKILLS_TREE.md
- agent-docs/meta-agent.example.md

## REQUIRED_SKILLS

(none)

## Location

`my-skills/episteme-skills/INTRO_TO_EPISTEME/`

## Original source URI

file:///Users/louisc/my-progs/episteme/my-skills/episteme-skills/INTRO_TO_EPISTEME/SKILL.md

## Integrity hash

(To be filled once the skill is stable and hashed.)
