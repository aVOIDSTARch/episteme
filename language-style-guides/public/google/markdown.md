...
3. Source text is readable and portable.

Contents:

Headings

1. Capitalization of titles and headers
2. Use a single H1 heading
3. Add spacing to headings
4. Use unique, complete names for headings
5. ATX-style headings

Lists

1. Nested list spacing
2. Use lazy numbering for long lists
...
2. Use fenced code blocks instead of indented code blocks
3. Escape newlines
...
1. Define reference links after their first use
2. Use reference links to reduce duplication
3. Use reference links for long links
...
1. On rare occasions, hold up submission if the CL actually makes the docs worse. It’s okay to ask the author to revert.
2. For substantial changes, start your own follow-up CL instead. Especially try to avoid comments of the form “You should also…”.
3. Prefer to suggest an alternative rather than leaving a vague comment.
4. When reasonable, LGTM immediately and trust that comments will be fixed appropriately.

As an author:

1. Cite the Better/Best Rule as often as needed.
2. Avoid wasting cycles with trivial argument. Capitulate early and move on.
...
`# Document title`: The first heading should be a level-one heading, ideally the same or nearly the same as the filename. The first level-one heading is used as the page` `.
...
`[TOC]`: if you use hosting that supports table of contents, such as Gitiles, put`[TOC]` after the short introduction. See [[TOC] documentation](https://gerrit.googlesource.com/gitiles/+/HEAD/Documentation/markdown.md#Table-of-contents).
...
Use a [[TOC] directive](https://gerrit.googlesource.com/gitiles/+/HEAD/Documentation/markdown.md#Table-of-contents) unless all of your content is above the fold1 on a laptop.
...
The [CommonMark spec](http://spec.commonmark.org/0.20/#hard-line-breaks) decrees that two spaces at the end of a line should insert a` ` tag. However, many directories have a presubmit check for trailing whitespace, and many IDEs will clean it up anyway.
...
Best practice is to avoid the need for a` ` altogether. A pair of newlines will create a paragraph tag; get used to that.
...
Every bit of HTML hacking reduces the readability and portability of our Markdown corpus. This in turn limits the usefulness of integrations with other tools, which may either present the source as plain text or render it. See [Philosophy](https://google.github.io/styleguide/docguide/philosophy.html).
